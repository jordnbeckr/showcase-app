...
function CompBlock({
          ...
          {event.couples.map(couple => {
            ...
            return (
              <div key={couple.studentId} className="px-3 py-1.5 flex items-center gap-2" style={{ backgroundColor: isSemiPhase && myMark ? '#f0fdf4' : 'var(--card)', minHeight: 40 }}>
                ...
                {floorLabel && (
                  <span className="text-xs px-1.5 py-0.5 flex-shrink-0" style={{ backgroundColor: '#f0fdfa', border: '1px solid #0d9488', borderRadius: 3, color: '#0d9488', fontWeight: 700 }}>{floorLabel}</span>
                )}
                <div className="flex gap-1.5 flex-wrap flex-shrink-0">
                  {isSemiPhase ? (
                    <button onClick={() => !callbackBlocked && onSemiMark(event.id, currentDance.heatId, couple.studentId)} disabled={callbackBlocked} className="px-4 py-1.5 text-sm font-semibold"
                      style={{ borderRadius: 6, border: '2px solid', borderColor: myMark ? '#16a34a' : callbackBlocked ? 'var(--border)' : '#6b21a8', backgroundColor: myMark ? '#dcfce7' : 'transparent', color: myMark ? '#14532d' : callbackBlocked ? '#ccc' : '#6b21a8', minWidth: 80, cursor: callbackBlocked ? 'not-allowed' : 'pointer', opacity: callbackBlocked ? 0.45 : 1 }}>
  ...
  const singleFull = markedCount >= event.semiSize
  return (
    <div className="rounded-lg overflow-hidden" style={{ border: '2px solid #d8b4fe' }}>
      <div className="px-4 py-2.5 flex items-center gap-3" style={{ backgroundColor: '#f3e8ff' }}>
        <span className="font-bold text-sm" style={{ color: '#6b21a8' }}>◆ {event.name}</span>
        {isSemiPhase ? (
          <span className="text-xs px-2 py-0.5 ml-auto font-semibold" style={{ backgroundColor: singleFull ? '#dcfce7' : '#fde68a', borderRadius: 3, color: singleFull ? '#14532d' : '#92400e' }}>
      ...
      <div className="divide-y" style={{ borderTop: '1px solid #e9d5ff' }}>
        {event.couples.map(couple => {
          const myPlace = compScores[`${event.id}-${singleHeatId}-${couple.studentId}`]
          ...
          return (
            <div key={couple.studentId} className="px-3 py-1.5 flex items-center gap-2" style={{ backgroundColor: isSemiPhase && myMark ? '#f0fdf4' : 'var(--card)', minHeight: 40 }}>
              ...
              <div className="flex gap-1.5 flex-wrap flex-shrink-0">
                {isSemiPhase ? (
                  <button onClick={() => !callbackBlocked && onSemiMark(event.id, singleHeatId, couple.studentId)} disabled={callbackBlocked} className="px-4 py-1.5 text-sm font-semibold"
                    style={{ borderRadius: 6, border: '2px solid', borderColor: myMark ? '#16a34a' : callbackBlocked ? 'var(--border)' : '#6b21a8', backgroundColor: myMark ? '#dcfce7' : 'transparent', color: myMark ? '#14532d' : callbackBlocked ? '#ccc' : '#6b21a8', minWidth: 80, cursor: callbackBlocked ? 'not-allowed' : 'pointer', opacity: callbackBlocked ? 0.45 : 1 }}>
                    {myMark ? '✓ Called' : 'Call back'}
                  </button>
                ) : (
